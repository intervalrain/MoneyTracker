    ...
    <View className="absolute top-0 left-0 right-0 bottom-0 bg-black/50 justify-center items-center z-50">
      <View className="bg-white p-4 rounded-2xl w-80">
        ...
        <TextInput
          ...
          onChangeText={(text) => setTypeData(prev => ({ ...prev, label: text }))}
        />
        ...
        >
          <MaterialCommunityIcons
            name={typeData.icon as keyof typeof MaterialCommunityIcons.glyphMap}
            size={24}
            color="black"
          />
          <Text>選擇圖標</Text>
          ...
          <View className="flex-row space-x-2">
            <Pressable
              ...
              className={`px-4 py-2 rounded-lg ${
                typeData.mode.includes('expense') ? 'bg-rain-500' : 'bg-rain-100'
              }`}
            >
              <Text className={`${
                typeData.mode.includes('expense') ? 'text-white' : 'text-black'
              }`}>支出</Text>
            </Pressable>            
            <Pressable
              onPress={() => handleModeToggle('income')}
              className={`px-4 py-2 rounded-lg ${
                typeData.mode.includes('income') ? 'bg-rain-500' : 'bg-rain-100'
              }`}
            >
              <Text className={`${
                typeData.mode.includes('income') ? 'text-white' : 'text-black'
              }`}>收入</Text>
            </Pressable>
          </View>
        </View>
      ...
      {showIconSelector && (
        <IconSelector
          currentIcon={typeData.icon as keyof typeof MaterialCommunityIcons.glyphMap}
          onSelect={(icon) => {
